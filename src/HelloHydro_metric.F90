...
subroutine get_4metric(CCTK_ARGUMENTS,i,j,k)
    ...
    gmunu(1,1) = -(alp*alp - beta2)
    gmunu(2,1) = 2.*betax(i,j,k)
    gmunu(3,1) = 2.*betay(i,j,k)
    gmunu(4,1) = 2.*betaz(i,j,k)
    gmunu(2,2) = gxx
    gmunu(2,3) = gxy
    gmunu(2,4) = gxz
    gmunu(3,2) = gyx
...
subroutine get_con(gmunu,gmunucon)
    real, intent(in) :: gmunu(4,4)
    real, intent(out) :: gmunucon(4,4)
    real :: det, det1 
...
subroutine get_sqrtg(gmunu,sqrtg)
    real, intent(in) :: gmunu(4,4)
